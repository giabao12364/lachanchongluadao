"""seed T_004 scam pattern

Revision ID: 2b03ad67bea2
Revises: 95f3c57a6169
Create Date: 2026-08-30 18:25:07.229928
...
revision: str = '2b03ad67bea2'
down_revision: Union[str, None] = '95f3c57a6169'
...
def upgrade() -> None:
    op.bulk_insert(scam_pattern_table, SCAM_PATTERNS)


def downgrade() -> None:
    ids = [item["id"] for item in SCAM_PATTERNS]

    op.execute(
        sa.text(
            """
            DELETE FROM scam_pattern
            WHERE id = ANY(:ids)
            """
        ).bindparams(
            sa.bindparam(
